...
namespace QuitSmoking.FE.Hoangnv.Controllers
{
    public class CreatePlanHoangnvController : Controller
    {
        ...
        public async Task<IActionResult> Index(string? title, int? currentPage, int? pageSize, string? motivation, int? dailySmoking)
        {
            ...
            {
                query += $"&motivation={System.Net.WebUtility.UrlEncode(motivation.Trim())}";
            }

            if (dailySmoking.HasValue)
            {
                query += $"&dailySmoking={dailySmoking}";
            }
        ...
        public async Task<IActionResult> Edit(int id)
        {
            ...
            return await Index(null, 1, 10, null, null);
        }
