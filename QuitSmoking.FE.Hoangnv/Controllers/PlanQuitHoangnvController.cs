    ...
    public class PlanQuitHoangnvController : Controller
    {
        ...
        public async Task<IActionResult> Create(int? id)
        {
            ...
            using (var httpClient = new HttpClient())
            {
                var tokenString = HttpContext.Request.Cookies.FirstOrDefault(c => c.Key == "TokenString").Value;
                httpClient.DefaultRequestHeaders.Add("Authorization", "Bearer " + tokenString);
                using (var response = await httpClient.GetAsync(APIEndPoint + "QuitMethodHoangnv/list"))
                {
                    if (response.IsSuccessStatusCode)
                    {
                        var content = await response.Content.ReadAsStringAsync();
                        var result = JsonConvert.DeserializeObject<List<QuitMethodHoangNv>>(content);
                        if (result != null)
                        {
                            ViewBag.QuitMethod = result;
                            return View();
                        }
                    }
                }
        ...
        public async Task<IActionResult> Create(PlanQuitMethodHoangNv model)
        {
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                else
                {
                    ...
                    // Gán lại ViewBag.QuitMethod trước khi return View(model)
                    using (var methodClient = new HttpClient())
                    {
                        methodClient.DefaultRequestHeaders.Add("Authorization", "Bearer " + tokenString);
                        var methodResponse = await methodClient.GetAsync(APIEndPoint + "QuitMethodHoangnv/list");
                        if (methodResponse.IsSuccessStatusCode)
                        {
                            var methodContent = await methodResponse.Content.ReadAsStringAsync();
                            var methodResult = JsonConvert.DeserializeObject<List<QuitMethodHoangNv>>(methodContent);
                            ViewBag.QuitMethod = methodResult ?? new List<QuitMethodHoangNv>();
                        }
                        else
                        {
                            ViewBag.QuitMethod = new List<QuitMethodHoangNv>();
                        }
                    }
                    return View(model);
                }
