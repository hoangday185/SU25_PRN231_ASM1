...
namespace QuitSmoking.API.HoangNV.Controllers
{
    ...
    public class CreatePlanHoangnvController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<PaginationResult<CreatePlanQuitSmokingHoangNv>> GetWithPaginate([FromQuery] int page, [FromQuery] int pageSize, [FromQuery] string? title, [FromQuery] string? motivation, [FromQuery] int? dailySmoking)
        {
            if (page < 1) page = 1; // Ensure page is at least 1
            if (pageSize < 1) pageSize = 10; // Ensure pageSize is at least 1
            return await _createPlanSerivce.GetPaginatedPlansAsync(page, pageSize, title, motivation, dailySmoking);
