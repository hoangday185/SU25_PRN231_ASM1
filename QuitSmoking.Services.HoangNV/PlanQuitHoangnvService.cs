...
namespace QuitSmoking.Services.HoangNV
{
    public class PlanQuitHoangnvService : IPlanQuitHoangNVService
    {
        ...
        public async Task<List<PlanQuitMethodHoangNv>> GetAllPlanQuitByCreateId(int createId)
        {
            return await _planQuitRepo.GetQueryable().Where(p => p.CreatePlanQuitSmokingHoangNvid == createId).ToListAsync();
        }
    }
}
