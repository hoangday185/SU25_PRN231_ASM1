...
namespace QuitSmoking.Services.HoangNV
{
    public class CreatePlanQuitSmokingHoangNvService : ICreatePlanQuitSmokingHoangNvService
    {
        ...
        public async Task<PaginationResult<CreatePlanQuitSmokingHoangNv>> GetPaginatedPlansAsync(int page = 1, int pageSize = 10, string? planTitle = null, string? motivation = null, int? dailySmoking = null)
        {
            ...
            {
                string keyword = motivation.Trim().ToLower();
                query = query.Where(p => p.MotivationReason.Contains(keyword));
            }

            if (dailySmoking.HasValue)
            {
                query = query.Where(p => p.DailyReductionGoal == dailySmoking);
            }
